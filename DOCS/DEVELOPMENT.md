...
Server 环境变量（`docker-compose.yml` 内置）：
```
...
```bash
# TypeScript 类型检查
docker compose -f docker-compose.yml exec server npx tsc --noEmit
...
```bash
# 在 frontend 容器内运行
docker compose -f docker-compose.yml exec frontend npx playwright test e2e-test.js
```
...
```bash
# 进入 server 容器 shell
docker compose -f docker-compose.yml exec server sh

# 进入 frontend 容器 shell
docker compose -f docker-compose.yml exec frontend sh

# 安装新的 server 依赖（修改 package.json 后需重建）
docker compose -f docker-compose.yml exec server npm install
